...
public class MathDoku extends Application {
    ...
    ArrayList<Integer> fullsolution = new ArrayList<>();
    ...
    public void start(Stage stage) {
        ...
        solver.setOnAction(e -> {
            ...
            boolean phewwwwwwww = false;
            int[][] solutionArray;
            ArrayList<Integer> blablabla = new ArrayList<>();
            ArrayList<ArrayList<Integer>> allpermu = new ArrayList<>();
            allpermu = permutations();

            while(!solved){

                boolean cellCorrect;
                solutionArray = matrixverson2(allpermu);
            ...
            phewwwwwwww = checkCellMistake(fullsolution);

            int[][] gidik = new int[4][4];

            for (int i = 0; i < 4; i++){
                for (int j = 0; j < 4; j++){
                    gidik[i][j] = fullsolution.get(i*4 + j);
                }
            }
            System.out.println(Arrays.deepToString(gidik));
            duplicateCorrect = checkDuplicate(gidik);*/
                if(cellCorrect){
                    solved = true;

                }
                System.out.println(solutionArrayList);
            /*if(!fullsolution.contains(0)){
                for (int i = 0; i < N; i++){
                    for (int j = 0; j < N; j++){
                        gidik[i][j] = 0;
                        fullsolution.set(i*N + j,0);
                    }
                }
            }*/
        ...
        commands.getChildren().addAll(load, undoredo, clear, showMistakes, fontChoiceBox, solver);
    ...
    private void drawGrid() {
        ...
        Rectangle bigrect = new Rectangle(width * numsquare, width * numsquare);
        //TODO if statement to see if cage is at border, then outer stroke of cage will become 8
    ...
    public int[][] matrix() {
        ...
        String[][] strings = new String[num.get()][num.get()];
        int count = 0;
        ...
        for (int i = 0; i < matrix.size(); i++) {
            for (int j = 0; j < matrix.get(i).size(); j++) {
                ...
                /*
                strings[i][j] = String.valueOf(solarr[i][j]);
                textFieldArrayList.set(count, new TextField(strings[i][j]));
                count++;
        ...
        System.out.println(Arrays.deepToString(solarr));
    ...
    public int[][] matrixverson2(ArrayList<ArrayList<Integer>> permutation){
    ...
    private boolean checkDuplicate(int[][] arrayOfInput){
        int[] row = new int[num.get()];
        int[] col = new int[num.get()];
        boolean dup = false;
        for (int i = 0; i < num.get(); i++){
